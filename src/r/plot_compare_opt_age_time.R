# plot_compare_opt_age_time.R: Visualise how optimised parameter sets depend on
# different vaccination rates, tagging strategies, age groups vaccinated and timing of vaccination.
...
sens_par_name_2 <- "n_age_vac"
trans_sens_par_2 <- \(x) {
  x
}
sens_par_title_2 <- "Livestock vaccinated"
sens_par_label_2 <- \(x) {
  ifelse(x == 2, "Young only", ifelse(x == 10, "All", x))
}
...
# Save the figure.
file_name <- paste("fig_compare_opt_age_timing", paste(strat_ids, collapse = "_"), sep = "_")
ggsave(
  filename = paste0(file_name, ".pdf"), plot = gg_cost, width = 12, height = 13, dpi = 600, scale = 1.1,
  path = "figures"
)
ggsave(
  filename = paste0(file_name, ".svg"), plot = gg_cost, width = 12, height = 13, dpi = 600, scale = 1.1,
  path = "figures"
)
